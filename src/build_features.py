"""Per-sample feature builder: 58 microstructure features per sample.

Aggregates the three raw files (transactions, order events, market bars) into
one feature row per sample in a constant-memory streaming pass. Trade/order/
book aggregates, multi-horizon market-bar returns (30/60/120/300s, 10min),
volume windows and acceleration, trend slopes, volatility, spread and book
imbalance means, 15/30s trade-flow imbalance, price dispersion, and
recency-weighted trade time.

Usage: python3 build_features.py <train|test> <n_samples> <data_dir> <out_dir>
"""
import sys, os, numpy as np, time, gc
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from stream_columns import ziter
...
if __name__ == '__main__':
    ...
    np.save(f'{out_dir}/X2_{split}.npy', X)
    np.save(f'{out_dir}/X2_{split}_keys.npy', np.array(keys))
