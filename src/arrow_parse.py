"""Minimal flatbuffers parser for Arrow IPC / feather files.

The competition data ships as single-batch LZ4-compressed feather files whose
decompressed record batches exceed the RAM of a small machine (2 GB). pyarrow
materializes the full batch, so we parse the IPC layout by hand and expose
byte ranges per column buffer for streaming decompression.
"""
import struct, os
