# DANN variant: RealMLP + gradient-reversal adversarial head (predict late-regime from shared repr).
# Goal: force regime-invariant representation to close the val->test transfer gap. Adv label: month>=40 within train.
...
late_tr = torch.tensor((month[tr] >= 40).astype(np.int64))
if device.type == 'cuda': late_tr = late_tr.to(device)
...
class RealMLP(nn.Module):
    def __init__(self, n_numerical, n_ens=8):
        ...
        self.adv_head = NTPLinear(n_ens, 128, 2)
...
for ep in range(EPOCHS):
    ...
    for i in range(0, len(ytr_t), BS):
        ...
        opt.zero_grad()
        lam = LAMBDA_ADV * progress
        pred, logits = model(Xtr_t[idx], lambd=lam)
        lab = late_tr[idx].unsqueeze(1).expand(-1, logits.shape[1]).reshape(-1)
        loss = loss_fn(pred, by) + lam * F.cross_entropy(logits.reshape(-1, 2), lab)
        loss.backward()
