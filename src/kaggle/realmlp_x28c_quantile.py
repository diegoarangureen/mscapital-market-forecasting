# X28c: champion (lambda_cos=1.0) with empirical-normal-quantile FEATURE preprocessing
# (ZWQ1037 EXP-TABM-011, CC BY 4.0: +0.001 both forward windows, better worst-month/Q25).
# Per fold: 1001 CDF knots fitted on <=200k TRAIN rows only, NaN -> train median, map -> ndtri.
# Screen: seed 2026 fold5 vs champion 0.17009, gate +0.004.
...
def fit_scale(X):
    # empirical-normal-quantile fit: 1001 knots per feature from <=200k rows (train only)
    rng = np.random.RandomState(0)
    n = X.shape[0]
    sub = X[rng.choice(n, 200000, replace=False)] if n > 200000 else X
    qs = np.linspace(0.0, 1.0, 1001)
    knots = np.nanquantile(sub, qs, axis=0).astype(np.float32)  # (1001, F)
    med = np.nanmedian(sub, axis=0).astype(np.float32)
    return knots, med

def apply_scale(X, knots, med):
    ...
    for j in range(F):
        x = X[:, j]
        x = np.where(np.isnan(x), med[j], x)
        u = np.searchsorted(knots[:, j], x, side='right') / float(nk)
        np.clip(u, 5e-4, 1.0 - 5e-4, out=u)
        out[:, j] = ndtri(u)
    return out
...
for seed in SEEDS:
    for fi, (va_mask, tr_mask) in enumerate(FOLDS):
        ...
        knots, med = fit_scale(X_all[tr_mask])
        Xtr = apply_scale(X_all[tr_mask], knots, med)
        Xva = apply_scale(X_all[va_mask], knots, med)
        ...
        for ep in range(EPOCHS):
            ...
            for i in range(0, len(ytr_t), BS):
                ...
                opt.zero_grad()
                loss = loss_fn(model(Xtr_t[idx]), by, lambda_cos=1.0)
                loss.backward()
        ...
        with torch.no_grad():
            ...
            Xte_s = apply_scale(Xte, knots, med)
