...
TAG = os.environ.get('TAG', 'advdrop')
...
# ---------- test prediction with best-epoch (holdout+ES) EMA state ----------
...
Xt2e = np.load(f'{XTRA}/X21_test.npy').astype(np.float32)
Xt3e = np.load(f'{XTRA2}/X22_test.npy').astype(np.float32)
Xte = np.concatenate([Xte, np.nan_to_num(Xt2e), np.nan_to_num(Xt3e)], axis=1)
...
np.save('/kaggle/working/test_pred_holdout.npy', pte)
