...
print('X21+X22 attached, advdrop ->', X_all.shape, flush=True)
...
TAG = os.environ.get('TAG', 'refit70advdrop')
...
Xte = np.concatenate([Xte, np.nan_to_num(np.load(f'{XTRA2}/X22_test.npy').astype(np.float32))], axis=1)
