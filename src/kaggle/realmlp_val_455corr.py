# RealMLP champion recipe + yunsuxiaozi rfmf-0726data 152 public features (-> 455f after advdrop).
...
# correlation pruning (yunsuxiaozi rfmf-realmlp style): drop |pearson|>=0.9 pairs, fit on train rows
tr_mask_pre = (np.load(f'{DATA}/full_month.npy') <= int(os.environ.get('TR_MAX', '60')))
X_tr_pre = X_all[tr_mask_pre]
C = np.corrcoef(np.nan_to_num(X_tr_pre[::7]), rowvar=False)  # subsample for speed
import numpy as _np
_np.fill_diagonal(C, 0)
drop_idx = set()
for j in range(C.shape[1]):
    if j in drop_idx: continue
    partners = _np.where(_np.abs(C[j]) >= 0.9)[0]
    for p in partners:
        if p != j and p not in drop_idx and p > j:
            drop_idx.add(int(p))
if drop_idx:
    keep = [i for i in range(X_all.shape[1]) if i not in drop_idx]
    X_all = X_all[:, keep]
    print('corr-pruned', len(drop_idx), 'cols ->', X_all.shape, flush=True)
del X_tr_pre, C
...
TAG = os.environ.get('TAG', 'x455corr')
