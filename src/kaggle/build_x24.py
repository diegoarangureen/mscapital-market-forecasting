# X24: absolute per-sample microstructure features (NO cross-sectional, NO month-relative).
...
# Runs on comp raw feathers mounted in-kernel. Output: X24_train.npy / X24_test.npy (~40 cols).
import os, time, gc
import numpy as np
...
def build(split):
    ...
    for sid, sbp, a1, b1, av1, bv1, a2, b2 in ziter(f'{DATA}/{split}/market.feather',
        ...
        n_m += np.bincount(lin, minlength=N3)
        spread += np.bincount(lin, weights=sp, minlength=N3)
        spread2 += np.bincount(lin, weights=sp*sp, minlength=N3)
        imb += np.bincount(lin, weights=im, minlength=N3)
        imb2 += np.bincount(lin, weights=im*im, minlength=N3)
        micro += np.bincount(lin, weights=mic, minlength=N3)
        microabs += np.bincount(lin, weights=np.abs(mic), minlength=N3)
        slopea2 += np.bincount(lin, weights=(a2.astype(f64)-a1f)/m, minlength=N3)
        # mid diffs within sample (approximate: diff vs previous row of same sample; rows sorted by sid,sbp)
        pm = prev_mid[sid]
        has = seen[sid]
        ...
        prev_mid[sid] = mid; seen[sid] = True
    ...
    np.save(f'/kaggle/working/X24_{split}.npy', X)
    print(split, 'X24 done', X.shape, round(time.time()-t0), 's', flush=True)
