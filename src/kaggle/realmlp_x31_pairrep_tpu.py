# X31-TPU 3-arm paired screen: base vs FLOW(18f) vs FLOW+X31(18f extension).
# X31 = FLOW-family extension (ofi_vol/accel/tw, qi2_tw, book_imb, dratio drift, ord vnet,
# gap stats, kyle, vpin) - built kernel-side by mscapital-x31-build. Same-session panel of
# 15 models cancels XLA session noise; incremental question = flow31 - flow (FLOW-only arm
# anchors cross-session comparison with the FLOW ablation result).
...
TAG = os.environ.get('TAG', 'x31pairrep')
...
Xb = X_all
X3n = np.load(data_file(XTRA3, 'X30_train_names.npy'), allow_pickle=True)
FLOW_IDX = [i for i, n in enumerate(X3n) if ('ofi' in str(n)) or str(n).startswith('x30_ord') or str(n).startswith('x30_qi') or ('depth_ratio' in str(n))]
assert len(FLOW_IDX) == 18, f'FLOW_IDX {len(FLOW_IDX)} != 18'
X3f = np.nan_to_num(X3[:, FLOW_IDX]); del X3
XTRA4 = os.environ.get('XTRA4', '/kaggle/input/mscapital-x31')
X4 = np.load(data_file(XTRA4, 'X31_train.npy')).astype(np.float32)
assert X4.shape[0] == Xb.shape[0], f'X31 rows {X4.shape[0]} != train rows {Xb.shape[0]}'
Xflow = np.concatenate([Xb, X3f], axis=1)
Xfx = np.concatenate([Xflow, np.nan_to_num(X4)], axis=1); del X4, X3f
print('3 arms: base', Xb.shape, 'flow', Xflow.shape, 'flow31', Xfx.shape, flush=True)
...
oof = {a: np.full(len(Xb), np.nan, dtype=np.float32) for a in ('base', 'flow', 'flow31')}
...
for seed in SEEDS:
    for arm, Xm in (('base', Xb), ('flow', Xflow), ('flow31', Xfx)):
     for fi, (va_mask, tr_mask) in enumerate(FOLDS):
        ...
        json.dump({'n_models_done': n_models, 'folds': fold_report,
                   **{f'oof_cos_61_70_{a}_partial': _oc(a) for a in ('base', 'flow', 'flow31')}},
...
print('OOF 61-70:', {a: res[f'oof_cos_61_70_{a}'] for a in ('base','flow','flow31')}, flush=True)
