# X28b: champion (lambda_cos=1.0) + RQ-KMeans auxiliary target-code head (3 layers x 3 codes, lambda_rq=0.1, yunsu rfmf-realmlp config reimplemented). Screen: seed 2026 fold5.
...
for seed in SEEDS:
    for fi, (va_mask, tr_mask) in enumerate(FOLDS):
        ...
        if device.type == 'cuda': yc_t = yc_t.to(device)
        ...
        for ep in range(EPOCHS):
            ...
            for i in range(0, len(ytr_t), BS):
                ...
                loss = loss_fn(reg_b, by, lambda_cos=1.0) + 0.1 * rq_ce(logits_b, yc_t[idx])
                loss.backward()
...
if cand:
    ...
    print('no competition submission.csv attached; skipping (screen run)', flush=True)
