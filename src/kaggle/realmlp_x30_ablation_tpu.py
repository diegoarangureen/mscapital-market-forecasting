...
TAG = os.environ.get('TAG', 'x30abl_'+os.environ.get('X30_KEEP','flow'))
...
XN = [str(x) for x in np.load(data_file(XTRA3, 'X30_train_names.npy'), allow_pickle=True)]
GROUPS = {
 'flow':  [0,1,2,3,4,5,6,7, 18,19,20,21, 22,23,24,25, 34,35],
 'vol':   [8,9,10,11,12, 16, 29,30],
 'price': [13,14,15, 17, 26,27,28, 31,32,33],
}
KEEP = os.environ.get('X30_KEEP', 'flow')
assert KEEP in GROUPS
X3 = X3[:, GROUPS[KEEP]]
print('X30 ablation keep', KEEP, len(GROUPS[KEEP]), 'features:', [XN[i] for i in GROUPS[KEEP]], flush=True)
...
oof = {'x30'+KEEP: np.full(len(Xb), np.nan, dtype=np.float32)}
...
for seed in SEEDS:
    for arm, Xm in (('x30'+KEEP, Xx),):
     for fi, (va_mask, tr_mask) in enumerate(FOLDS):
        ...
        _c = oof[arm]; _seen = ~np.isnan(_c); _mm = _m & _seen
        json.dump({'n_models_done': n_models, 'folds': fold_report,
                   'oof_cos_61_70_partial': cos_np(_c[_mm], y_all[_mm]) if _mm.any() else None},
...
a = 'x30'+KEEP
res['oof_cos_61_70'] = cos_np(oof[a][m6170], y_all[m6170])
res['oof_cos_66_70'] = cos_np(oof[a][late66], y_all[late66])
json.dump(res, open(f'/kaggle/working/metrics_{TAG}.json','w'), indent=1)
print('OOF 61-70:', res['oof_cos_61_70'], flush=True)
