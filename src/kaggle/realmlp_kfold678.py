...
xs_dir = None
for cand in ['/kaggle/input/mscapital-build-xs75b', '/kaggle/input/kernels/diegoaranguren/mscapital-build-xs75b']:
...
if xs_dir is None:
    ...
    if ls: xs_dir = str(Path(ls[0]).parent)
assert xs_dir, 'XS75 not found'
XS_tr = np.load(f'{xs_dir}/XS_train.npy').astype(np.float32)
assert XS_tr.shape[0] == X_all.shape[0]
X_all = np.concatenate([X_all, np.nan_to_num(XS_tr)], axis=1); del XS_tr
print('train 530f ->', X_all.shape, flush=True)
X23_tr = np.load(f'{xs_dir.replace("xs75b","x23") if False else xs_dir}/../mscapital-build-x23/X23_train.npy') if False else None
x23_dir = None
...
X23_tr = np.load(f'{x23_dir}/X23_train.npy').astype(np.float32)
assert X23_tr.shape[0] == X_all.shape[0]
X_all = np.concatenate([X_all, np.nan_to_num(X23_tr)], axis=1); del X23_tr
print('train 678f ->', X_all.shape, flush=True)
...
XS_te = np.load(f'{xs_dir}/XS_test.npy').astype(np.float32)
assert XS_te.shape[0] == Xte.shape[0]
Xte = np.concatenate([Xte, np.nan_to_num(XS_te)], axis=1); del XS_te
print('test 530f ->', Xte.shape, flush=True)
X23_te = np.load(f'{x23_dir}/X23_test.npy').astype(np.float32)
assert X23_te.shape[0] == Xte.shape[0]
Xte = np.concatenate([Xte, np.nan_to_num(X23_te)], axis=1); del X23_te
print('test 678f ->', Xte.shape, flush=True)
