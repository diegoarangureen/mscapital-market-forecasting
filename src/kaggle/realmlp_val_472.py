# RealMLP champion recipe + 0726 (-> 455f) + X25 17 trade arrival/impact features -> 472f.
...
x25_dir = None
for cand in ['/kaggle/input/datasets/diegoaranguren/mscapital-x25', '/kaggle/input/mscapital-x25']:
    if Path(cand).exists() and (Path(cand)/'X25_train.npy').exists():
        x25_dir = cand; break
if x25_dir is None:
    import subprocess
    r = subprocess.run(['find','/kaggle/input','-name','X25_train.npy','-maxdepth','6'], capture_output=True, text=True, timeout=60)
    ls = [l for l in r.stdout.strip().split('\n') if l.strip()]
    if ls: x25_dir = str(Path(ls[0]).parent)
assert x25_dir, 'X25 not found'
X25 = np.load(f'{x25_dir}/X25_train.npy').astype(np.float32)
assert X25.shape[0] == X_all.shape[0]
X_all = np.concatenate([X_all, np.nan_to_num(X25)], axis=1)
del X25
print('+X25 ->', X_all.shape, flush=True)
