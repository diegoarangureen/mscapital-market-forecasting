# RealMLP champion recipe + 0726 (-> 455f) + X26 event/volume-sync features (OFI/VPIN family) -> 469f.
# Single-variable test vs champion val 0.1671 (455f). tr<=60/val 61-70, cos loss.
...
# ---------- X26 event/volume-sync microstructure features ----------
x26_dir = None
for cand in ['/kaggle/input/datasets/diegoaranguren/mscapital-x26', '/kaggle/input/mscapital-x26']:
    if Path(cand).exists() and (Path(cand)/'X26_train.npy').exists():
        x26_dir = cand; break
if x26_dir is None:
    import subprocess
    r = subprocess.run(['find','/kaggle/input','-name','X26_train.npy','-maxdepth','6'], capture_output=True, text=True, timeout=60)
    ls = [l for l in r.stdout.strip().split('\n') if l.strip()]
    if ls: x26_dir = str(Path(ls[0]).parent)
assert x26_dir, 'X26 not found'
X26 = np.load(f'{x26_dir}/X26_train.npy').astype(np.float32)
assert X26.shape[0] == X_all.shape[0]
X_all = np.concatenate([X_all, np.nan_to_num(X26)], axis=1)
del X26
print('+X26 ->', X_all.shape, flush=True)
...
# ---- multi-metric panel (from src/kaggle/panel.py) ----
...
report_panel(best_pv, yva, month[va], tag=f'{TAG} best')
