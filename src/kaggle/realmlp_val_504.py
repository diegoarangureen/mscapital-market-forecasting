# RealMLP champion recipe + 0726 152 public features (-> 455f) + X24 41 absolute microstructure features -> 496f.
...
x24_dir = None
for cand in ['/kaggle/input/datasets/diegoaranguren/mscapital-x24c', '/kaggle/input/mscapital-x24c']:
    if Path(cand).exists() and (Path(cand)/'X24c_train.npy').exists():
        x24_dir = cand; break
if x24_dir is None:
    import subprocess
    r = subprocess.run(['find','/kaggle/input','-name','X24c_train.npy','-maxdepth','6'], capture_output=True, text=True, timeout=60)
    ls = [l for l in r.stdout.strip().split('\n') if l.strip()]
    if ls: x24_dir = str(Path(ls[0]).parent)
assert x24_dir, 'X24 not found'
X24 = np.load(f'{x24_dir}/X24c_train.npy').astype(np.float32)
