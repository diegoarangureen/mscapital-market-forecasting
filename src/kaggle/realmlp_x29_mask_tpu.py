# X29-TPU: price==0 empty-book MASKING A/B (parent-approved direction (a), Sep 23 22:46).
# Hypothesis: L1 price-derived terms (spread, microprice dev, slopes) are artifacts of stale/zero
# quotes when the book level is empty; masking them to 0 removes noise. Features: X21m/X22m
# (built kernel-side from raw streams by mscapital-x29-masked-build v1; L1 price terms masked
...
TAG = os.environ.get('TAG', 'x29masktpu_panel')
DATA = os.environ.get('DATA', '/kaggle/input/mscapital-matrices')
XTRA = os.environ.get('XTRA', '/kaggle/input/mscapital-x21m')
XTRA2 = os.environ.get('XTRA2', '/kaggle/input/mscapital-x22m')
...
X1 = np.load(data_file(XTRA, 'X21m_train.npy')).astype(np.float32)
X2 = np.load(data_file(XTRA2, 'X22m_train.npy')).astype(np.float32)
