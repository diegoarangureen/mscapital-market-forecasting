...
def agg_ofi(path, ns):
    ...
    for sid, sbp, a1, b1, av1, bv1, a2, b2, av2, bv2 in ziter(path, cols):
        ...
        # bid side
        contrib += np.where(db>0, bv1[1:], np.where(db<0, -bv1[:-1], np.maximum(dvb,0)*0 + dvb*0))  # placeholder replaced below
        contrib = np.zeros(n-1)
        bid_up = db>0; bid_dn = db<0
        ...
        ask_up = da>0; ask_dn = da<0
        contrib += np.where(ask_up, av1[:-1].astype(np.float64), 0.0)   # ask price up = less pressure... CKS: ask up -> +q^a_{n-1}? 
        contrib = contrib - np.where(ask_dn, av1[1:].astype(np.float64), 0.0) - np.where(ask_up, 0.0, 0.0)
...
def agg_kyle(path, ns):
    ...
    F['kyle_corr']=cov/np.maximum(np.sqrt(var*np.maximum(syy if False else 1,1e-12)),1e-12)
    del F['kyle_corr']
    return F
