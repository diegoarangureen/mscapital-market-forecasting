# E2: recency-weighted GBM, evaluated on full val AND late val (66-70)
...
# linear ramp: weight 0.5 at month 0 -> 1.5 at month 60
w_tr = 0.5 + month[tr]/60.0
dva = lgb.Dataset(X[va], label=y[va], feature_name=keys)
for tag, w in (('ramp', w_tr),):
    vp=[]
    for seed in (7,42,123):
        dtr = lgb.Dataset(X[tr], label=y[tr], feature_name=keys, weight=w)
        ...
        print(f'{tag} seed {seed}: {cos(vp[-1],y[va]):.6f} ({m.best_iteration} it)', flush=True)
        del m, dtr; gc.collect()
    avg = np.mean(vp,0); u = unit(avg); lv = late[va]
    print(f'{tag} seedavg full centered: {cos(u,y[va]):.6f}  LATE: {cos(u[lv],y[va][lv]):.6f}', flush=True)
    np.save(f'/tmp/work/recency2_{tag}_val.npy', avg)
print('reference: baseline full 0.126330, late 0.128253', flush=True)
