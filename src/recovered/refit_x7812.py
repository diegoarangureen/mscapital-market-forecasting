...
if which in ('trainmodels','trainpred'):
    ...
    X = np.empty((n,182), np.float32); X[:,:d0]=p0; del p0; gc.collect()
    col=d0
    for nm in ('X7','X8','X9','X12'):
...
iters = [109, 97, 102]  # 1.1x of [99,88,~93]
if which=='trainmodels':
    for seed,nr in zip((7,42,123), iters):
        ...
        m.save_model(f'/tmp/work/x7812_seed{seed}.txt')
    ...
    Xt = np.empty((nt,182), np.float32); Xt[:,:d0]=p0t; del p0t; gc.collect()
    col=d0
    for nm in ('X7','X8','X9','X12'):
    ...
    for seed in (7,42,123):
        m = lgb.Booster(model_file=f'/tmp/work/x7812_seed{seed}.txt')
        out = np.empty(nt)
    ...
    np.save('/tmp/work/refit_x7812_test.npy', np.mean(preds,0))
    print('X7812_REFIT_TEST_DONE', flush=True)
    ...
    for seed in (7,42,123):
        ...
        preds.append(out); print('seed',seed,'predicted',flush=True); del m,out; gc.collect()
    np.save('/tmp/work/refit_x7812_train.npy', np.mean(preds,0))
    print('X7812_REFIT_TRAIN_DONE', flush=True)
