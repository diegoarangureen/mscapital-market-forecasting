...
def run(seed):
    ...
    net = nn.Sequential(nn.Linear(D,512), nn.ReLU(), nn.Dropout(0.2),
    ...
    for ep in range(30):
        ...
        if c>best: best=c; bestp=pv; bad=0; be=ep
            ...
            bad+=1
            if bad>=4: break
    global be_global; be_global=be
    return bestp, best
import json
vp=[]; bes={}
for seed in (0,1):
    p,b=run(seed); vp.append(p); bes[str(seed)]=be_global; print('mlp seed',seed,'best',round(b,6),flush=True)
json.dump(bes, open('/tmp/work/mlpbig_best_epochs.json','w'))
print('best epochs:', bes, flush=True)
m=np.mean(vp,0)
u=(m-m.mean())/m.std()
print('MLP seedavg:', round(cos(m,yv),6), 'centered:', round(cos(u,yv),6), flush=True)
np.save('/tmp/work/mlpbig_seedavg_val.npy', m)
g = np.load('/tmp/work/x2base_seedavg_val.npy')
def unit(v):
    v=v-v.mean(); sd=v.std(); return v/(sd if sd>0 else 1)
late = month>=66
lv = late[va]
gu, mu2 = unit(g), unit(m)
print('corr big-mlp gbm:', round(float(np.corrcoef(mu2,gu)[0,1]),4), flush=True)
for wm in (0.4, 0.5, 0.6):
    bl = (1-wm)*gu + wm*mu2
    print(f'blend big-mlp w_mlp={wm}: full {cos(bl,yv):.6f} late {cos(bl[lv],yv[lv]):.6f}', flush=True)
print('MLPBIG_DONE', flush=True)
