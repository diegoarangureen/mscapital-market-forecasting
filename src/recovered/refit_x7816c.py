...
if which in ('trainmodels','trainpred'):
    ...
    X = np.memmap('/tmp/work/x7816b_all.f32', dtype=np.float32, mode='w+', shape=(n,246))
    ...
    X.flush()
    print('train matrix ready (memmap)', flush=True)
...
iters = [122, 156, 139, 129, 134]  # 1.1x of xall16 [111,142,~126]; 5 seeds
if which=='trainmodels':
    import os
    for seed,nr in zip((7,42,123,2024,31337), iters):
        if os.path.exists(f'/tmp/work/x7816b_seed{seed}.txt'):
            print('skip seed',seed,'(exists)',flush=True); continue
        p=dict(params); p['seed']=seed
        ds=lgb.Dataset(X,y); m=lgb.train(p,ds,num_boost_round=nr)
        m.save_model(f'/tmp/work/x7816b_seed{seed}.txt')
    ...
    Xt = np.empty((nt,246), np.float32); Xt[:,:d0]=p0t; del p0t; gc.collect()
    col=d0
    for nm in ('X7','X8','X9','X12','X13','X16'):
    ...
    preds=[]
    for seed in (7,42,123,2024,31337):
        m = lgb.Booster(model_file=f'/tmp/work/x7816b_seed{seed}.txt')
        out = np.empty(nt)
    ...
    np.save('/tmp/work/refit_x7816b_test.npy', np.mean(preds,0))
    print('X7816B_REFIT_TEST_DONE', flush=True)
    ...
    for seed in (7,42,123,2024,31337):
        ...
        preds.append(out); print('seed',seed,'predicted',flush=True); del m,out; gc.collect()
    np.save('/tmp/work/refit_x7816b_train.npy', np.mean(preds,0))
    print('X7816B_REFIT_TRAIN_DONE', flush=True)
