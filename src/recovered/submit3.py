...
path=sys.argv[1] if len(sys.argv)>1 and sys.argv[1].endswith('.csv') else '/tmp/work/submission_blend.csv'; size=os.path.getsize(path)
c=KaggleClient()
...
sr.submission_description=sys.argv[2] if len(sys.argv)>2 else 'blend'
