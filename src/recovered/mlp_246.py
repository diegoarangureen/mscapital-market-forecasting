# MLP eval: pure 246 GBM feats (X2+X7+X8+X9+X12+X13+X16), no fp views. Val tr<=60/va>=61.
...
n = len(X2)
D = 246
M = np.memmap(f'{W}/mlp246.f32', dtype=np.float32, mode='w+', shape=(n, D))
...
pv = np.mean(preds,0); np.save(f'{W}/mlp246_val.npy', pv)
u = unit(pv)
...
g = unit(np.load(f'{W}/xall16_val.npy'))
