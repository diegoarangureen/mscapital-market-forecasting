...
def collect(split, ns):
    R = np.zeros((ns, NG), np.float32)
    V = np.zeros((ns, NG), np.float32)
    ...
    sump = np.zeros((ns, NG), np.float64)
    sumv = np.zeros((ns, NG), np.float64)
    for sid, sbp, ap, tv in ziter(f'/tmp/mscapital/{split}/market.feather',
        ...
        np.add.at(sump, (sid, g), ap.astype(np.float64))
        np.add.at(sumv, (sid, g), tv.astype(np.float64))
