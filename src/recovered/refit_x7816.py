...
if which in ('trainmodels','trainpred'):
    ...
    X = np.memmap('/tmp/work/x7816_all.f32', dtype=np.float32, mode='w+', shape=(n,246))
...
iters = [122, 156, 139]  # 1.1x of xall16 [111,142,~126]
if which=='trainmodels':
    for seed,nr in zip((7,42,123), iters):
        ...
        m.save_model(f'/tmp/work/x7816_seed{seed}.txt')
    ...
    for seed in (7,42,123):
        m = lgb.Booster(model_file=f'/tmp/work/x7816_seed{seed}.txt')
        out = np.empty(nt)
    ...
    np.save('/tmp/work/refit_x7816_test.npy', np.mean(preds,0))
    print('X7816_REFIT_TEST_DONE', flush=True)
    ...
    for seed in (7,42,123):
        ...
        preds.append(out); print('seed',seed,'predicted',flush=True); del m,out; gc.collect()
    np.save('/tmp/work/refit_x7816_train.npy', np.mean(preds,0))
    print('X7816_REFIT_TRAIN_DONE', flush=True)
