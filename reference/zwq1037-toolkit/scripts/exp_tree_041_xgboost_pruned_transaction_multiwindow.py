"""Test three pruned transaction multi-window features on robust EXP-TREE-037."""
...
PRUNED_TRANSACTION_MULTIWINDOW_FEATURES = [
...
def main() -> None:
    ...
    transaction_multiwindow = pd.read_feather(
        processed_dir / "train_transaction_multiwindow_features.feather",
        columns=["sample_id", *PRUNED_TRANSACTION_MULTIWINDOW_FEATURES],
    )
    ...
    del transaction_gaps, transaction_multiwindow, labels
    gc.collect()
    ...
    feature_columns = exp037_features + PRUNED_TRANSACTION_MULTIWINDOW_FEATURES
    if len(exp037_features) != 159 or len(feature_columns) != 162:
        raise AssertionError("Unexpected EXP-TREE-041 feature schema.")
    if len(feature_columns) != len(set(feature_columns)):
        raise AssertionError("Duplicate feature names in EXP-TREE-041.")
    ...
    evaluate_and_save(
        ...
        experiment_id="EXP-TREE-041",
        description=(
            "add pruned 10-second signed amount ratio and 10/30-second trade-count "
            "imbalance features to robust EXP037"
        ),
        model=model,
