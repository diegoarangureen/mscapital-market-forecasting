"""Fit the validated Relative319+XS40+order20 TabM on all 0-70 months."""
...
RUN_NAME = "tabm_relative319_xs40_order20_seed137_fulltrain"
...
def main() -> None:
    ...
    train_features = np.concatenate(
        [train_base, train_xs, train_order[ORDER_COLUMNS].to_numpy(dtype=np.float32)], axis=1
    )
    del train_base, train_xs, train_order
    gc.collect()
    ...
    test_features = np.concatenate(
        [test_base, test_xs, test_order[ORDER_COLUMNS].to_numpy(dtype=np.float32)], axis=1
    )
    del test_base, test_xs, test_order
    gc.collect()
    if train_features.shape[1] != 379 or test_features.shape[1] != 379:
    ...
    if preprocessing_path.exists():
        ...
        np.savez_compressed(
            ...
            feature_columns=np.asarray([*train_columns, *xs_columns, *ORDER_COLUMNS]),
        )
    ...
    torch.save(model.state_dict(), RUN_DIR / "model_fulltrain.pt")
    ...
    metadata = {
        ...
        "validation_50_59": 0.15192334671506483,
        "validation_62_70_no66": 0.1588192105208768,
