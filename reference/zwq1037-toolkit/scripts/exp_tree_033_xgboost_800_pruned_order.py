"""Confirm the two robust EXP-TREE-025 improvements together."""
...
def main() -> None:
    ...
    if len(feature_columns) != 155 or len(feature_columns) != len(set(feature_columns)):
        raise AssertionError("EXP-TREE-033 must contain exactly 155 unique features.")
    if set(feature_columns).intersection(ORDER_FEATURE_COLUMNS):
        raise AssertionError("Legacy eight-column order group was not fully removed.")
    ...
    parameters = model_parameters(n_estimators=800)
    ...
    evaluate_and_save(
        ...
        experiment_id="EXP-TREE-033",
        description="combine two validated changes: 800 trees and removal of legacy order eight",
        model=model,
