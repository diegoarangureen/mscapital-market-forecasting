"""Test Order20 on Relative319 without the harmful XS40 block."""
...
EXPERIMENT_ID = "EXP-TREE-078-RELATIVE319-ORDER20"
...
def main() -> None:
    ...
    del order
    feature_columns = [*baseline_columns, *RELATIVE_COLUMNS, *ORDER20_COLUMNS]
    if len(feature_columns) != 339 or len(set(feature_columns)) != 339:
        raise AssertionError("Expected 339 unique features")
    ...
    baseline = pd.read_feather(
        PROJECT / "data/interim/tree_experiments/EXP-TREE-069-RELATIVE319/train059_valid6070/validation_predictions.feather",
        columns=["sample_id", "month", "xgb_relative319"],
    )
    baseline = baseline.loc[(baseline["month"] >= 62) & (baseline["month"] != 66)]
    if not np.array_equal(ids, baseline["sample_id"].to_numpy()):
        raise AssertionError("Tree319 baseline IDs do not align")
    baseline_prediction = baseline["xgb_relative319"].to_numpy(dtype=np.float64)
    scores = {
        "baseline_tree319": cosine(target, baseline_prediction),
        "candidate_tree339": cosine(target, prediction),
    }
    scores["delta"] = scores["candidate_tree339"] - scores["baseline_tree319"]
    result = {
    ...
    pd.DataFrame({
        ...
        "target": target,
        "baseline_tree319": baseline_prediction,
        "candidate_tree339": prediction,
