...
def main():
    ...
    selected = (months >= 50) & (months <= 59)
    score_mask = np.ones(int(selected.sum()), dtype=bool)
    ...
    run_dir = PROJECT_DIR / "data" / "interim" / "submissions" / "tabm_relative319_xs40_seed42_holdout049"
    ...
    model.load_state_dict(torch.load(run_dir / "model_holdout049.pt", map_location=device, weights_only=True))
    amp_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    indices = np.arange(len(valid_base))
    target = labels["target"].to_numpy(dtype=np.float32)
    target_std = float(target[months <= 49].std(dtype=np.float64))
    predictions = {}
    ...
    out = PROJECT_DIR / "data" / "interim" / "tree_experiments" / "XS40-EQUAL-BLOCKS-FIRST-WINDOW"
