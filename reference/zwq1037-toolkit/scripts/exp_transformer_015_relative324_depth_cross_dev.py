"""Test five order-book and aggressive-trade cross features on Joint-Transformer."""
...
EXPERIMENT_ID = "EXP-TRANSFORMER-015-RELATIVE324-DEPTH-CROSS-DEV"
SEED = 42
TRAIN_END_MONTH = 49
...
def load_relative324(
    project_dir: Path,
    labels: pd.DataFrame,
) -> tuple[np.ndarray, list[str]]:
    """Append five precomputed trade-depth interactions to Relative319."""
    ...
    cross_data = pd.read_feather(
        ...
        columns=["sample_id", *CROSS_FEATURE_COLUMNS],
    ).sort_values("sample_id").reset_index(drop=True)
    if not np.array_equal(
        cross_data["sample_id"].to_numpy(), labels["sample_id"].to_numpy()
    ):
        raise AssertionError("Cross features and sequence labels are not aligned.")
    cross_features = cross_data[CROSS_FEATURE_COLUMNS].to_numpy(
        dtype=np.float32, copy=True
    )
    features = np.concatenate([relative319, cross_features], axis=1)
    columns = [*relative319_columns, *CROSS_FEATURE_COLUMNS]
    if features.shape[1] != 324 or len(columns) != 324 or len(set(columns)) != 324:
        raise AssertionError("Expected 324 unique Relative324 features.")
...
def main() -> None:
    ...
    static_features, feature_columns = load_relative324(project_dir, labels)
    relative_end = len(feature_columns) - len(CROSS_FEATURE_COLUMNS)
    relative_start = relative_end - len(RELATIVE_COLUMNS)
    if (
        feature_columns[relative_start:relative_end] != RELATIVE_COLUMNS
        or feature_columns[-len(CROSS_FEATURE_COLUMNS):] != CROSS_FEATURE_COLUMNS
    ):
        raise AssertionError("Relative324 schema differs.")
