"""Paired TabM validation for twenty order-at-quote position features."""
...
EXPERIMENT_ID = "EXP-TABM-026-ORDER-QUOTE-POSITION20"
...
def main() -> None:
    ...
    # XS40 remains frozen so the only experimental change is the new 20 columns.
    ...
    if len(set([*base_columns, *xs_columns, *FEATURE_COLUMNS])) != 379:
        raise AssertionError("Expected 379 unique candidate columns.")
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        # Reuse the identical seed42 baseline trained immediately before EXP026.
        baseline_dir = (
            PROJECT_DIR
            / "data"
            / "interim"
            / "tree_experiments"
            / "EXP-TABM-025-TRADE-QUOTE-STATE16"
            / fold_name
        )
        baseline = json.loads(
            (baseline_dir / "baseline_result.json").read_text(encoding="utf-8")
        )
        ...
        baseline["prediction"] = baseline_predictions["baseline"].to_numpy(
            dtype=np.float64, copy=True
        )
        del baseline_predictions

        candidate_features = np.concatenate([base, xs40, state_values], axis=1)
        if candidate_features.shape[1] != 379:
            raise AssertionError("Expected 379 candidate columns.")
        candidate = recipe.run_model(
            f"{fold_name}_relative319_xs40_order20",
            candidate_features,
            target,
            months,
            train_end,
    ...
    summary = {
        ...
        "candidate": "baseline + 20 order-at-quote position features",
        "baseline_feature_count": 359,
        "candidate_feature_count": 379,
