"""Prune EXP043 to its ten strongest training-selected cross-feature ranks."""
...
# 由 EXP043 在 0～59 月训练得到的新增特征重要性排序选出，不使用验证标签。
# Selected from EXP043's added-feature importance trained on months 0-59 only.
PRUNED_CROSS_RANK_FEATURES = [
    "cross_rank_top_19",
    "cross_rank_top_16",
    "cross_rank_top_13",
    "cross_rank_top_14",
...
def main() -> None:
    ...
    if len(exp037_features) != 159:
        raise AssertionError("Unexpected EXP-TREE-044 base feature schema.")
    model_data = add_cross_feature_ranks(model_data)
    feature_columns = exp037_features + PRUNED_CROSS_RANK_FEATURES
    if len(feature_columns) != 169 or len(feature_columns) != len(set(feature_columns)):
        raise AssertionError("Unexpected EXP-TREE-044 final feature schema.")
    ...
    evaluate_and_save(
        ...
        experiment_id="EXP-TREE-044",
        description=(
            "retain only the ten strongest training-selected cross-rank columns from EXP043"
        ),
        model=model,
