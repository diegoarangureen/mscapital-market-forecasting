"""Replace cached book features with exact full-sequence reconstructions."""
...
def main() -> None:
    ...
    # 用完整原始序列重算值替换旧的同名盘口特征，特征集合本身保持不变。
    # Replace cached book values with full-sequence values while keeping the schema fixed.
    missing_book_features = [
        feature for feature in BOOK_FEATURE_COLUMNS if feature not in base_features
    ]
    if missing_book_features:
        raise AssertionError(
            f"Base table is missing book features: {missing_book_features}"
        )
    exact_book = pd.read_feather(
        processed_dir / "train_market_book_features_exact.feather"
    )
    model_data = model_data.drop(columns=BOOK_FEATURE_COLUMNS).merge(
        exact_book,
        on="sample_id",
        how="left",
        validate="one_to_one",
    ...
    if len(feature_columns) != 307:
        raise AssertionError("Unexpected EXP059 feature count.")
    if len(feature_columns) != len(set(feature_columns)):
        raise AssertionError("Duplicate features entered EXP059.")
    ...
    print("training EXP-TREE-059 exact full-sequence book replacement", flush=True)
    run_one(
        project_dir=project_dir,
        experiment_id="EXP-TREE-059",
        description=(
            "EXP053R with all 35 cached book microstructure values replaced by "
            "exact reconstructions from the complete raw market sequence"
        ),
