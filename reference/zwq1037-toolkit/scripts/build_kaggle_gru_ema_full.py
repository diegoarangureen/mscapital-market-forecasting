...
src = root / r"data\interim\kaggle_kernels\multistream_factorized_transformer_fulltrain\run.py"
out = src.parent / "run_v2_gru_ema_full.py"
text = src.read_text(encoding="utf-8")
text = text.replace('/kaggle/working/factorized_gru_fulltrain', '/kaggle/working/gru_ema_full')
...
def main():
    ...
    print(f"gru_ema_full epochs={EPOCHS}, ema={EMA_DECAY}, batch_size={BATCH_SIZE}, visible_gpu_count={_VISIBLE_GPU_COUNT}, gpu_names={_VISIBLE_GPU_NAMES}", flush=True)
    ...
    logs = []
    start_epoch = 1
    checkpoint_path = WORK_DIR / "gru_ema_training_checkpoint.pt"
    ...
    model_path = WORK_DIR / "timeaware_gru379_ema0999_full_e5.pt"
...
metadata_path = src.parent / 'kernel-metadata.json'
metadata = json.loads(metadata_path.read_text(encoding='utf-8'))
metadata['code_file'] = nb_path.name
metadata['title'] = 'MultiStream GRU Transformer EMA Full Train'
