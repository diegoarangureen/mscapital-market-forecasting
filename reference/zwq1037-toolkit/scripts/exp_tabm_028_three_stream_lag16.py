...
from build_three_stream_lag_features import FEATURE_COLUMNS


EXPERIMENT_ID = "EXP-TABM-028-THREE-STREAM-LAG16"
...
def main() -> None:
    ...
    state = pd.read_feather(
        ...
        / "data"
        / "processed"
        / "train_three_stream_lag_features.feather"
    ).sort_values("sample_id").reset_index(drop=True)
    ...
    if len(set([*base_columns, *xs_columns, *ORDER_COLUMNS, *FEATURE_COLUMNS])) != 395:
        raise AssertionError("Expected 395 unique candidate columns.")
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        if candidate_features.shape[1] != 395:
            raise AssertionError("Expected 395 candidate columns.")
        candidate = recipe.run_model(
            f"{fold_name}_relative319_xs40_order20_lag16",
            candidate_features,
            target,
            months,
            train_end,
    ...
    summary = {
        ...
        "candidate": "baseline + 16 three-stream lag features",
        "baseline_feature_count": 379,
        "candidate_feature_count": 395,
