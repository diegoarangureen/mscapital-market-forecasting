...
PUBLIC_CACHE = PROJECT_DIR / "data/interim/public_features/rfmf_0726data/numpy_cache"
...
def main() -> None:
    ...
    public_names = json.loads((PUBLIC_CACHE / "feature_columns.json").read_text(encoding="utf-8"))
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        candidate_features = np.concatenate([base, xs40, order_values, state_values, public_extra], axis=1)
        if candidate_features.shape[1] != 397:
