"""Paired TabM validation for sixteen trade-at-quote state features."""
...
from build_trade_quote_state_features import FEATURE_COLUMNS


EXPERIMENT_ID = "EXP-TABM-025-TRADE-QUOTE-STATE16"
...
def main() -> None:
    ...
    # XS40 remains frozen so the only experimental change is the new 16 columns.
    # 固定原有 XS40，确保实验唯一变量是新增的16列。
    xs40, xs_columns = recipe.add_relative_features(base, months, base_columns)
    state = pd.read_feather(
        PROJECT_DIR
        / "data"
        / "processed"
        / "train_trade_quote_state_features.feather"
    ).sort_values("sample_id").reset_index(drop=True)
    if list(state.columns) != ["sample_id", *FEATURE_COLUMNS]:
        raise AssertionError("Unexpected trade-quote feature columns.")
    if not np.array_equal(sample_ids, state["sample_id"].to_numpy()):
        raise AssertionError("Trade-quote feature sample IDs do not match labels.")
    state_values = state[FEATURE_COLUMNS].to_numpy(dtype=np.float32, copy=True)
    del state, labels
    gc.collect()

    if len(set([*base_columns, *xs_columns, *FEATURE_COLUMNS])) != 375:
        raise AssertionError("Expected 375 unique candidate columns.")
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        baseline_features = np.concatenate([base, xs40], axis=1)
        if baseline_features.shape[1] != 359:
            raise AssertionError("Expected 359 baseline columns.")
        baseline = recipe.run_model(
            f"{fold_name}_relative319_xs40",
            baseline_features,
            target,
            months,
            train_end,
        ...
        candidate_features = np.concatenate([base, xs40, state_values], axis=1)
        if candidate_features.shape[1] != 375:
            raise AssertionError("Expected 375 candidate columns.")
        candidate = recipe.run_model(
            f"{fold_name}_relative319_xs40_state16",
            candidate_features,
            target,
            months,
            train_end,
    ...
    summary = {
        ...
        "baseline": "Relative319 + frozen XS40, seed42, 15 epochs",
        "candidate": "baseline + 16 trade-at-quote state features",
        "baseline_feature_count": 359,
        "candidate_feature_count": 375,
