...
RUN = ROOT / "tree_experiments/EXP-REALMLP-003-YUNSU-MEMBERWISE-COSINE"
FOLDS = [
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for start in range(0, len(shuffled), BATCH_SIZE):
                ...
                with torch.autocast("cuda", dtype=torch.float16):
                    member_prediction = model(x).float()
            ...
            with torch.no_grad():
                for start in range(0, len(valid_idx), BATCH_SIZE):
                    ...
                    with torch.autocast("cuda", dtype=torch.float16):
                        chunks.append(model(gpu_features[gpu_idx]).mean(dim=1).float().cpu().numpy())
