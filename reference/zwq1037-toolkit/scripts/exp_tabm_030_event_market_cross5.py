"""Paired TabM validation for five trade-flow versus book-depth features."""
...
from build_event_market_cross_features import CROSS_FEATURE_COLUMNS as FEATURE_COLUMNS


EXPERIMENT_ID = "EXP-TABM-030-EVENT-MARKET-CROSS5"
...
def main() -> None:
    ...
    # XS40 remains frozen so the only experimental change is the new 5 columns.
    # 固定原有 XS40，确保实验唯一变量是新增的5列。
    xs40, xs_columns = recipe.add_relative_features(base, months, base_columns)
    state = pd.read_feather(
        PROJECT_DIR
        / "data"
        / "processed"
        / "train_event_market_cross_features.feather"
    ).sort_values("sample_id").reset_index(drop=True)
    ...
    if len(set([*base_columns, *xs_columns, *ORDER_COLUMNS, *FEATURE_COLUMNS])) != 384:
        raise AssertionError("Expected 384 unique candidate columns.")
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        if candidate_features.shape[1] != 384:
            raise AssertionError("Expected 384 candidate columns.")
        candidate = recipe.run_model(
            f"{fold_name}_relative319_xs40_order20_event_market_cross5",
            candidate_features,
            target,
            months,
            train_end,
    ...
    summary = {
        ...
        "candidate": "baseline + 5 trade-flow versus book-depth features",
        "baseline_feature_count": 379,
        "candidate_feature_count": 384,
