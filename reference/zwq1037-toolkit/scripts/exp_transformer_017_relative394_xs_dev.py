"""Test 75 cross-sectional features from a public high-scoring TabM recipe."""
...
EXPERIMENT_ID = "EXP-TRANSFORMER-017-RELATIVE394-XS-DEV"
SEED = 42
TRAIN_END_MONTH = 49
VALID_START_MONTH = 50
VALID_END_MONTH = 59
EXCLUDED_VALID_MONTHS: tuple[int, ...] = ()
...
WEIGHT_DECAY = 1.0e-4
SMOOTH_L1_WEIGHT = 0.35
COSINE_WEIGHT = 0.65

def finite_percentile(values: np.ndarray) -> np.ndarray:
    """Return centered percentile ranks while preserving missing values."""

    output = np.full(len(values), np.nan, dtype=np.float32)
    valid = np.isfinite(values)
    if valid.any():
        output[valid] = (
            rankdata(values[valid], method="average") / valid.sum() - 0.5
        ).astype(np.float32)
    return output


def build_cross_feature_ranks(
    selected_features: np.ndarray,
    training_rows: int,
) -> np.ndarray:
    """Rank the selected feature values within every sample."""

    training_medians = np.nanmedian(selected_features[:training_rows], axis=0)
    training_medians = np.nan_to_num(training_medians, nan=0.0)
    clean = np.where(np.isfinite(selected_features), selected_features, training_medians)
    output = np.empty(clean.shape, dtype=np.float32)
    column_ranks = np.arange(clean.shape[1], dtype=np.float32)
    column_ranks /= max(clean.shape[1] - 1, 1)
    for start in range(0, len(clean), 50_000):
        end = min(start + 50_000, len(clean))
        order = np.argsort(clean[start:end], axis=1)
        ranks = np.empty(order.shape, dtype=np.float32)
        rows = np.arange(end - start)[:, None]
        ranks[rows, order] = column_ranks
        output[start:end] = ranks - 0.5
    return output


def load_relative394(
    project_dir: Path,
    labels: pd.DataFrame,
    train_end: int,
    run_dir: Path,
) -> tuple[np.ndarray, list[str]]:
    """Add 30 monthly ranks, 15 monthly z-scores and 30 row-wise ranks."""

    relative319, relative319_columns = gru.load_relative319(project_dir, labels)
    selection_path = run_dir / "xs_feature_selection.npz"
    if selection_path.exists():
        saved = np.load(selection_path)
        top30 = saved["top30"].astype(np.int64)
    else:
        sample_size = min(200_000, train_end)
        sample_indices = np.random.default_rng(SEED).choice(
            train_end, sample_size, replace=False
        )
        target = labels["target"].to_numpy(dtype=np.float32)
        dataset = lgb.Dataset(
            relative319[sample_indices],
            label=target[sample_indices],
            free_raw_data=True,
        )
        selector = lgb.train(
            {
        ...
        importance = selector.feature_importance(importance_type="gain")
        top30 = np.argsort(importance)[::-1][:30].astype(np.int64)
        np.savez_compressed(
            selection_path,
            top30=top30,
            top30_columns=np.asarray([relative319_columns[i] for i in top30]),
            selection_training_rows=np.asarray([train_end]),
        )
        del selector, dataset, sample_indices
        gc.collect()

    months = labels["month"].to_numpy()
    selected = relative319[:, top30]
    monthly_percentile = np.full(selected.shape, np.nan, dtype=np.float32)
    monthly_zscore = np.full((len(selected), 15), np.nan, dtype=np.float32)
    for month in np.unique(months):
        indices = np.flatnonzero(months == month)
        month_values = selected[indices]
        for column_index in range(30):
            monthly_percentile[indices, column_index] = finite_percentile(
                month_values[:, column_index]
            )
        for column_index in range(15):
            values = month_values[:, column_index]
            valid = np.isfinite(values)
            if valid.any():
                mean = values[valid].mean(dtype=np.float64)
                scale = values[valid].std(dtype=np.float64) + 1.0e-8
                month_output = np.full(len(values), np.nan, dtype=np.float32)
                month_output[valid] = ((values[valid] - mean) / scale).astype(np.float32)
                monthly_zscore[indices, column_index] = month_output

    cross_feature_rank = build_cross_feature_ranks(selected, train_end)
    xs_features = np.concatenate(
        [monthly_percentile, monthly_zscore, cross_feature_rank], axis=1
    )
    features = np.concatenate([relative319, xs_features], axis=1)
    top30_columns = [relative319_columns[i] for i in top30]
    columns = [
        *relative319_columns,
        *[f"month_percentile__{name}" for name in top30_columns],
        *[f"month_zscore__{name}" for name in top30_columns[:15]],
        *[f"row_feature_rank__{name}" for name in top30_columns],
    ]
    if features.shape[1] != 394 or len(columns) != 394 or len(set(columns)) != 394:
        raise AssertionError("Expected 394 unique Relative394 features.")
...
def main() -> None:
    ...
    validation_ids = labels.iloc[valid_start:valid_end]["sample_id"].to_numpy()
    baseline_path = (
        project_dir / "data" / "interim" / "tree_experiments"
        / "EXP-TABM-016-RELATIVE-SCALE-ADD12" / gru.FOLD_NAME
        / "validation_predictions.feather"
    )
    baseline_frame = pd.read_feather(
        baseline_path, columns=["sample_id", "b001_trim1"]
    ).set_index("sample_id")
    if not np.isin(validation_ids, baseline_frame.index.to_numpy()).all():
        raise AssertionError("Some validation IDs are missing from the baseline.")
    baseline = baseline_frame.loc[validation_ids, "b001_trim1"].to_numpy(dtype=np.float64)
    ...
    static_features, feature_columns = load_relative394(
        project_dir, labels, train_end, run_dir
    )
    if (
        len(feature_columns) != 394
        or feature_columns[319].startswith("month_percentile__") is False
        or feature_columns[349].startswith("month_zscore__") is False
        or feature_columns[364].startswith("row_feature_rank__") is False
    ):
        raise AssertionError("Relative394 schema differs.")
    ...
    validation_scored = ~validation_rows["month"].isin(EXCLUDED_VALID_MONTHS).to_numpy()
    if not validation_scored.any():
        raise AssertionError("No scored validation rows remain.")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if epoch in EVALUATION_EPOCHS:
            ...
            validation_target_all = validation_rows["target"].to_numpy(dtype=np.float64)
            validation_months_all = validation_rows["month"].to_numpy()
            validation_target = validation_target_all[validation_scored]
            validation_months = validation_months_all[validation_scored]
            candidate_scored = candidate[validation_scored]
            blended_scored = blended[validation_scored]
            baseline_scored = baseline[validation_scored]
            standalone_metrics = evaluate(
                validation_target, candidate_scored, validation_months,
                VALID_START_MONTH, VALID_END_MONTH,
            )
            blend_metrics = evaluate(
                validation_target, blended_scored, validation_months,
                VALID_START_MONTH, VALID_END_MONTH,
            )
            baseline_metrics = evaluate(
                validation_target, baseline_scored, validation_months,
                VALID_START_MONTH, VALID_END_MONTH,
            )
            ...
            output["is_scored"] = validation_scored
    ...
    result = {
        ...
        "excluded_validation_months": list(EXCLUDED_VALID_MONTHS),
        "training_rows": train_end,
        "validation_rows": int(validation_scored.sum()),
