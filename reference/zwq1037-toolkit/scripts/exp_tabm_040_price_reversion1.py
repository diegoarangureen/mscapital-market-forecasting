...
RECOVERY_COLUMNS = ["price_reversion_fraction_15"]


EXPERIMENT_ID = "EXP-TABM-040-PRICE-REVERSION1"
...
def main() -> None:
    ...
    recovery_frame = pd.read_feather(
        ...
        columns=["sample_id", *RECOVERY_COLUMNS],
    ).sort_values("sample_id").reset_index(drop=True)
    ...
    if len(set([*base_columns, *xs_columns, *ORDER_COLUMNS, *FEATURE_COLUMNS, *RECOVERY_COLUMNS])) != 386:
        raise AssertionError("Expected 386 unique candidate columns.")
    ...
    for fold_name, (train_end, valid_start, valid_end, score_key) in FOLDS.items():
        ...
        if candidate_features.shape[1] != 386:
            raise AssertionError("Expected 386 candidate columns.")
    ...
    summary = {
        ...
        "candidate": "TabM385 k32 plus price-shock 15-second reversion feature",
        "baseline_feature_count": 385,
        "candidate_feature_count": 386,
        "single_variable_change": "append target-free price-reversion feature only",
        ...
        "passed": passed,
        "next_action": (
            "paired_early_window" if passed else "reject_price_reversion_feature"
        ),
    }
