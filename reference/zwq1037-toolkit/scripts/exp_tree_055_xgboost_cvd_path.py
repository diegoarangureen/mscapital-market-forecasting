"""Test two CVD path features independently against exact EXP053R."""
...
from build_transaction_cvd_path_features import FEATURE_COLUMNS as CVD_FEATURE_COLUMNS
...
def main() -> None:
    ...
    cvd = pd.read_feather(processed_dir / "train_transaction_cvd_path_features.feather")
    ...
    model_data = (
        ...
        .merge(cvd, on="sample_id", how="left", validate="one_to_one")
    ...
    del gap, last15, cvd, public, labels
    gc.collect()
    ...
    candidate_features = control_features + CVD_FEATURE_COLUMNS
    if len(control_features) != 307 or len(candidate_features) != 309:
        raise AssertionError("Unexpected EXP055 feature count.")
    if len(candidate_features) != len(set(candidate_features)):
        raise AssertionError("Duplicate features entered EXP055.")
    ...
    print("training EXP-TREE-055 CVD path candidate", flush=True)
    run_one(
        project_dir=project_dir,
        experiment_id="EXP-TREE-055",
        description=(
            "EXP053R plus normalized final-60-second CVD path range and pullback "
            "from the path high"
        ),
