"""Combine separately validated transaction and order event-gap features."""
...
from build_order_event_gap_features import FEATURE_COLUMNS as ORDER_GAP_FEATURES
...
def main() -> None:
    ...
    transaction_gap = pd.read_feather(
        processed_dir / "train_transaction_event_gap_features.feather"
    )
    order_gap = pd.read_feather(processed_dir / "train_order_event_gap_features.feather")
    labels = pd.read_feather(project_dir / "data" / "raw" / "label.feather")
    model_data = (
        model_data.merge(transaction_gap, on="sample_id", how="left", validate="one_to_one")
        .merge(order_gap, on="sample_id", how="left", validate="one_to_one")
        .merge(labels, on="sample_id", how="inner", validate="one_to_one")
    )
    del transaction_gap, order_gap, labels
    gc.collect()

    baseline_features = base_features + TRANSACTION_FEATURE_COLUMNS + ORDER_MULTI_FEATURE_COLUMNS
    feature_columns = baseline_features + TRANSACTION_GAP_FEATURES + ORDER_GAP_FEATURES
    if len(baseline_features) != 155 or len(feature_columns) != 163:
        raise AssertionError("Unexpected EXP-TREE-039 feature schema.")
    ...
    evaluate_and_save(
        ...
        experiment_id="EXP-TREE-039",
        description="combine four transaction-gap and four order-gap features on EXP034",
        model=model,
