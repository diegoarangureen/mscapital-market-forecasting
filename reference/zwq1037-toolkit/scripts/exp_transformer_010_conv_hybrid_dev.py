...
EXPERIMENT_ID = "EXP-TRANSFORMER-010-CONV-HYBRID-DEV"
SEED = 42
TRAIN_END_MONTH = 49
...
class TemporalConvBlock(nn.Module):
    def __init__(self, kernel_size: int) -> None:
        ...
        self.network = nn.Sequential(
            nn.Conv1d(D_MODEL, D_MODEL, kernel_size, padding=kernel_size // 2, bias=False),
            nn.GELU(),
    ...
    def forward(self, tokens: torch.Tensor, valid_prefix: torch.Tensor) -> torch.Tensor:
        residual = self.network(tokens.transpose(1, 2)).transpose(1, 2)
        return (tokens + residual) * valid_prefix[:, :, None]
...
class TransformerSequenceEncoder(nn.Module):
    def __init__(self, input_size: int) -> None:
        ...
        self.conv5 = TemporalConvBlock(5)
        self.conv3 = TemporalConvBlock(3)
    ...
    def forward(self, sequence: torch.Tensor) -> torch.Tensor:
        ...
        tokens = tokens * valid_prefix[:, :, None]
        tokens = self.conv5(tokens, valid_prefix)
        tokens = self.conv3(tokens, valid_prefix)
