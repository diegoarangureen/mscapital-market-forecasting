"""Evaluate prefix tree counts from the trained EXP067 LightGBM model."""
...
def main() -> None:
    ...
    if len(feature_columns) != 307:
        raise AssertionError("Unexpected EXP067 iteration-check feature count.")
    ...
    if base_config["feature_columns"] != feature_columns:
        raise AssertionError("EXP067 model feature order differs from current data.")
    model_path = project_dir / "outputs" / "models" / "exp-tree-067.joblib"
    model = joblib.load(model_path)

    for iteration in ITERATIONS:
        experiment_id = f"EXP-TREE-067-N{iteration}"
        ...
        parameters["prediction_iteration"] = iteration
        metadata = evaluate_and_save(
            project_dir=project_dir,
            experiment_id=experiment_id,
            description=(
                f"Prefix evaluation of the trained EXP067 LightGBM model using "
                f"only its first {iteration} of 800 trees"
            ),
            model=model,
        ...
        metadata["shared_training_run"] = "EXP-TREE-067"
        metadata["prediction_iteration"] = iteration
