...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, position):
        ...
        for source in ('transaction', 'order'):
            raw = np.asarray(self.events[source][row], dtype=np.float32)
...
def main():
    ...
    train_loader = DataLoader(SequenceDataset(train_rows, market, static, events, lengths, norm, target, scale), batch_size=BATCH_SIZE, shuffle=True, num_workers=0, pin_memory=True, drop_last=True)
    valid_loader = DataLoader(SequenceDataset(valid_rows, market, static, events, lengths, norm, target, scale), batch_size=BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=True)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        log = {'epoch': epoch, 'loss': total / count, 'cosine': score, 'seconds': time.time() - started}
        logs.append(log)
        print(json.dumps(log), flush=True)
        if score > best:
            best = score
            torch.save(model.state_dict(), RUN / 'best_model.pt')
            pd.DataFrame({'sample_id': ids[valid_rows], 'month': months[valid_rows], 'target': target[valid_rows], 'prediction': validation}).to_feather(RUN / 'validation_predictions.feather')
        compact = {'experiment': 'EXP-SEQUENCE-020-SUBSECOND-GRU-TRANSFORMER', 'best_cosine': best, 'baseline': OLD_BASELINE, 'delta': best - OLD_BASELINE, 'epochs_finished': epoch, 'logs': logs, 'passed': best >= OLD_BASELINE + .001}
        (RUN / 'score_only.json').write_text(json.dumps(compact, indent=2), encoding='utf-8')
        if epoch == 1 and log['seconds'] * EPOCHS > 7200:
            print('runtime_budget_exceeded_no_expansion', flush=True)
            break
