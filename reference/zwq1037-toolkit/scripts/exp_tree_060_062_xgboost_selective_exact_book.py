"""Test three selective exact-book replacements with cached-value fallback."""
...
CANDIDATES = [
    ("EXP-TREE-060", "ofi_1_last"),
    ("EXP-TREE-061", "microprice_displacement_last"),
    ("EXP-TREE-062", "book_imbalance_2_std_robust"),
]
...
def main() -> None:
    ...
    candidate_features = [feature for _, feature in CANDIDATES]
    ...
    exact_book = pd.read_feather(
        ...
        columns=["sample_id", *candidate_features],
    ).rename(columns={feature: f"exact__{feature}" for feature in candidate_features})
    ...
    if len(feature_columns) != 307:
        raise AssertionError("Unexpected selective replacement feature count.")
    if len(feature_columns) != len(set(feature_columns)):
        raise AssertionError("Duplicate features entered the selective experiments.")
    ...
    for experiment_id, feature in CANDIDATES:
        cached_values = model_data[feature].to_numpy(copy=True)
        exact_values = model_data[f"exact__{feature}"].to_numpy(dtype=np.float64)
        valid_exact = np.isfinite(exact_values)
        replacement_values = cached_values.copy()
        replacement_values[valid_exact] = exact_values[valid_exact]
        changed_fraction = float(
            (~np.isclose(cached_values, replacement_values, rtol=1e-5, atol=1e-8)).mean()
        )
        model_data[feature] = replacement_values
        print(
            f"training {experiment_id}: {feature}, changed_fraction={changed_fraction:.6f}",
            flush=True,
        )
        run_one(
            ...
            description=(
                f"EXP053R with only {feature} replaced by the exact full-sequence "
                "value; cached value retained where the exact value is missing"
            ),
        ...
        # 每轮后恢复基线列，确保三个实验彼此独立。
        # Restore the baseline column so each experiment remains independent.
        model_data[feature] = cached_values
        gc.collect()
