"""Test four leakage-safe q90 large-transaction features on EXP-TREE-034."""
...
from build_transaction_large_trade_features import FEATURE_COLUMNS
...
def main() -> None:
    ...
    model_data = (
        model_data.merge(large_trade, on="sample_id", how="left", validate="one_to_one")
        .merge(labels, on="sample_id", how="inner", validate="one_to_one")
    )
    del large_trade, labels
    gc.collect()
    ...
    if len(baseline_features) != 155 or len(feature_columns) != 159:
        raise AssertionError("Unexpected EXP-TREE-036 feature schema.")
    ...
    evaluate_and_save(
        ...
        experiment_id="EXP-TREE-036",
        description="add four q90 large-transaction imbalance and share features to EXP034",
        model=model,
