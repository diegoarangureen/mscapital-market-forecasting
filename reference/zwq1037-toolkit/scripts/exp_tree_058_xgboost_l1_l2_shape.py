"""Test four time-weighted L1-L2 shape features against EXP053R."""
...
from build_market_l1_l2_shape_features import FEATURE_COLUMNS as SHAPE_FEATURE_COLUMNS
...
from train_full_xgboost_exp051_submission import load_public_table, selected_public_features
...
def main() -> None:
    ...
    shape = pd.read_feather(processed_dir / "train_market_l1_l2_shape_features.feather")
    ...
    model_data = (
        ...
        .merge(shape, on="sample_id", how="left", validate="one_to_one")
    ...
    del gap, last15, shape, public, labels
    gc.collect()
    ...
    candidate_features = control_features + SHAPE_FEATURE_COLUMNS
    if len(control_features) != 307 or len(candidate_features) != 311:
        raise AssertionError("Unexpected EXP058 feature count.")
    if len(candidate_features) != len(set(candidate_features)):
        raise AssertionError("Duplicate features entered EXP058.")
    ...
    print("training EXP-TREE-058 L1-L2 shape candidate", flush=True)
    run_one(
        project_dir=project_dir,
        experiment_id="EXP-TREE-058",
        description=(
            "EXP053R plus four real-time-weighted final-60/15-second L1-L2 depth "
            "share, imbalance gap, and directional disagreement features"
        ),
