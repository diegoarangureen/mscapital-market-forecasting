...
def load(fold, start, end):
    ...
    for x in (t42,t137,j42,j137,h42): assert np.array_equal(x.sample_id.to_numpy(),ids)
    ...
    h=h42.prediction.to_numpy(float); y=tabm.target.to_numpy(float); m=tabm.month.to_numpy()
