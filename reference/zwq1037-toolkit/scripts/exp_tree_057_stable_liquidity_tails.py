"""Test three stable within-sample liquidity tail features against EXP053R."""
...
from build_stable_liquidity_tail_features import FEATURE_COLUMNS as TAIL_FEATURE_COLUMNS
...
def main() -> None:
    ...
    tails = pd.read_feather(
        processed_dir / "train_stable_liquidity_tail_shape_features.feather"
    )
    ...
    model_data = (
        ...
        .merge(tails, on="sample_id", how="left", validate="one_to_one")
    ...
    del gap, last15, tails, public, labels
    gc.collect()
    ...
    candidate_features = control_features + TAIL_FEATURE_COLUMNS
    if len(control_features) != 307 or len(candidate_features) != 310:
        raise AssertionError("Unexpected EXP057 feature count.")
    if len(candidate_features) != len(set(candidate_features)):
        raise AssertionError("Duplicate features entered EXP057.")
    ...
    print("training EXP-TREE-057 stable-liquidity-tail candidate", flush=True)
    run_one(
        project_dir=project_dir,
        experiment_id="EXP-TREE-057",
        description=(
            "EXP053R plus exact within-sample mid-normalized spread-tail excess, "
            "depth-drought ratio, and transaction-volume concentration"
        ),
